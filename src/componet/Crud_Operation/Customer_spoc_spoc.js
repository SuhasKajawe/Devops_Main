...
const Customer_spoc_spoc = () => {
  ...
  const fetchCustomerDetails = async () => {
    try {
      const response = await axios.get('http://192.168.10.42:3001/show_customer_spoc_spoc');
  ...
  const handleUpdateCustomer = async () => {
    try {
      await axios.put(`http://192.168.10.42:3001/update_Customer_SPOC/${selectedCustomer.customer_spoc_master_id}`, formData);
      setIsModalOpen(false);
      window.location.href = '/show_Customer_SPOC';
      fetchCustomerDetails();
  ...
  const handleDeleteCustomer = async (customerId) => {
    try {
      await axios.put(`http://192.168.10.42:3001/delete_customer_SPOC/${customerId}`); 
      window.location.href = '/show_Customer_SPOC';
      fetchCustomerDetails();
    } catch (error) {
      ...
      <div className="customer-details" style={{ border: '2px solid black', borderRadius: '20px', padding: '20px' }}>
        <div className="customer-details">
          <div className='row align-items-center'>
            <div className='col'>
              <label htmlFor="search" className="search-label"><h6> Customer SPOC Name:</h6> <span>      </span></label>
            </div>
        ...
        <table className="customer-table">
          <thead>
            <tr>
              ...
              <th style={{ width: '170px' }}>SPOC ID</th>
          ...
          <tbody>
            {currentCustomers.filter(customer => customer.name.toLowerCase().includes(searchTerm.toLowerCase())).map((customer ,index)=> (
              <tr key={customer.customer_spoc_master_id}>
                <td>{index+1}</td>
                <td>{customer.customer_spoc_master_id}</td>
                <td>{customer.name}</td>
                ...
                <td className="action-buttons">
                  <button className="edit-button" onClick={() => handleEditCustomer(customer)}>
                    ...
                    <span className="hover-text1"  >Update Customer Spoc</span>
                  </button>
                  <span style={{ margin: '0 10px' }}></span>
                  <button className="delete-button" onClick={() => handleDeleteCustomer(customer.customer_spoc_master_id)}>
                    <FontAwesomeIcon icon={faUserTimes} style={{ fontSize: '24px', color: 'blue' }} />
                    <span className="hover-text1"   >Disable Customer Spoc</span>
                  </button>
                </td>
              </tr>
            ))}
  ...
  <div className="modal" style={{ display: 'block', backgroundColor: 'rgba(0,0,0,0.5)', position: 'fixed', top: 0, left: 0, right: 0, bottom: 0 }}>
    <div className="modal-content" style={{ backgroundColor: '#fff', margin: '15% auto', padding: '20px', width: '50%', border: '2px solid black', borderRadius: '20px', }}>
      <h2 style={{ textAlign: 'center', marginBottom: '20px' , border: '2px solid black', borderRadius: '20px', padding: '20px' }}>{isEditing ? 'Edit Customer SPOC' : 'Update Customer SPOC'}</h2>
      <hr/>
      <form onSubmit={handleUpdateCustomer} style={{ display: 'flex', flexDirection: 'column' }}>
        <div style={{ display: 'flex', flexWrap: 'wrap', marginBottom: '15px' }}>
          {Object.keys(formData).map(key => (
            !['customer_spoc_master_id'].includes(key) && (
...
export default Customer_spoc_spoc;
