...
const NewCustomerCreation = () => {
  ...
  const handleClose2 = () => setShowForm(false);
  ...
  const handleAddSelectedCustomer = () => {
    if (selectedCustomer) {
      ...
      if (selectedCustomerObject) {
        ...
        const customerSpocId = selectedCustomerObject.customer_spoc_master_id;
  ...
  const handleRemoveCustomer = index => {
    setSelectedCustomers(prevSelectedCustomers => {
      ...
      const selectedCustomerIds = updatedSelectedCustomers.map(customer => customer.customer_spoc_master_id);
  ...
  const handleSubmitButtonClick = async () => {
    try {
      ...
      formDataToSubmit.append('partnerName', selectedpartnerid);
      //-----------------------------------
      const response = await fetch('http://192.168.10.42:3001/onsubmit', {
        method: 'POST',
  ...
  const handleSubmitButtonClickinDB = async () => {
    ...
    try {
      ...
      formDataToSubmit.append('partnerName', selectedpartnerid);
      //-----------------------------------
      const response = await fetch('http://192.168.10.42:3001/StoreinDBCustomer', {
        method: 'POST',
        body: formDataToSubmit, // Send FormData directly as the body
      });
      setLoading1(false); 
  ...
  const handleCancel = () => {
    setShowPartnerModal(false); // Close the partner modal
  };
  const [showPartnerModal, setShowPartnerModal] = useState(false);
  const [selectedPartnerName, setSelectedPartnerName] = useState('');
  const handleClose = () => setShowPartnerModal(false); 
  const handleProceedWithoutPartner = (event) => {
    // Handle the logic for proceeding without selecting a partner
    const selectedPartnerId = event.target.value;
   
    const selectedPartnerObject = partners.find(partner => partner.partner_id === selectedPartnerId);
  
    if (selectedPartnerObject) {
      setSelectedPartner('');
     
      console.log('Selected Partner:', selectedPartnerObject.name);
    } else {
      setSelectedPartner(''); // If no partner is selected or found, clear the selectedPartner state
      console.log('No partner selected');
    }
  
    setShowPartnerModal(false); // Close the modal
    setSelectedPartnerName(''); // Reset selected partner name
  ...
  const handleAddPartner = () => {
    // Navigate to the page where the user can add a new partner
    window.location.href = '/NewPartnerCreation';
  };

  const handleopenpart = () => setShowPartnerModal(true)

  
      const handlePartnerSelection = (event) => {
          const selectedPartnerId = event.target.value;
          setSelectedpartnerid(selectedPartnerId); // Set the selected partner ID state value
          console.log('Selected partner ID:', selectedPartnerId); // Log selected partner ID
          console.log('Partners array:', partners); // Log partners array
  
          const selectedPartnerObject = partners.find(partner => partner.partner_id === selectedPartnerId);
          console.log('Selected partner object:', selectedPartnerObject); // Log selected partner object
          
          if (selectedPartnerObject) {
              setSelectedPartner(selectedPartnerObject.name);
              console.log('Selected Partner:', selectedPartnerObject.name);
          } else {
              setSelectedPartner('');
              console.log('No partner selected');
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const customerResponse = await fetch('http://192.168.10.42:3001/customersRe');
        ...
        const aeSpocResponse = await fetch('http://192.168.10.42:3001/customersAeSpoc');
        ...
        // Fetch partner data
        const partnerResponse = await fetch('http://192.168.10.42:3001/PartnerDetails');
        ...
        setLoading(false); // Set loading to false only after all fetch operations succeed
  
        // Set showPartnerModal to true only if there's no selected partner
 ...
 // // Handle customer selection
  ...
  const selectedCustomerIds = updatedSelectedCustomers.map(name => {
    ...
    return customer.customer_spoc_master_id;
  });
  ...
  const handleSubmit = async e => {
    ...
    setLoading2(true);
    try {
      const response = await fetch('http://192.168.10.42:3001/createCustomer', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
      ...
      setShowForm(false); // Hide the form setLoading2(true);
      setLoading2(false);
...
// Define the function to handle AE SPOC selection from the dropdown
...
// Define the function to handle adding selected AE SPOC to the list
    ...
    if (selectedAeSpoc) {
        ...
        if (selectedAeSpocObject) {
            ...
            if (!uniqueAeSpocs.includes(selectedAeSpocObject)) {
                ...
                const aeSpocId = selectedAeSpocObject.customer_ae_spoc_master_id; // Assuming customer_ae_spoc_master_id is the correct property
                setFormData(prevFormData => ({
                    ...prevFormData,
...
// Define the function to handle removing AE SPOC
// Define the function to handle removing AE SPOC
// Define the function to handle removing AE SPOC
// Function to handle removing AE SPOC
// Function to handle removing AE SPOC
const handleRemoveAeSpoc = index => {
  ...
  setUniqueAeSpocs(prevUniqueAeSpocs => {
    ...
    const removedAeSpocId = removedAeSpoc?.customer_ae_spoc_master_id; // Use optional chaining to avoid errors if AE SPOC is undefined
    // Update the state with the modified array
    setFormData(prevFormData => {
      if (removedAeSpocId && typeof prevFormData.Ae_Spoc_id === 'string') {
        ...
        // Update Ae_Spoc_id in the form data if it has changed
        return { ...prevFormData, Ae_Spoc_id: updatedAeSpocIds }; // Update Ae_Spoc_id in the form data
      }
      return prevFormData; // Return the original form data if AE SPOC ID is not found or Ae_Spoc_id is not a string
    });
...
const AEhandleSubmit = async e => {
    ...
    setLoading3(true);
    try {
      const response = await fetch('http://192.168.10.42:3001/createrAESpoc', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
...
<div className="spinner"></div>
  ...
  <div className="col ">
    <div className="form-group d-flex align-items-center">
      <label className="form-label" htmlFor="customerName" style={{ minWidth: "130px" }}>Customer Name:<span className="text-danger">*</span></label>
      <input
        type="text"
        name="customerName"
...
<Modal show={showForm1} onHide={handleClose1} dialogClassName="modal-lg">
  <Modal.Header >
    <Modal.Title>Add New Customer AE SPOC</Modal.Title>
  </Modal.Header>
  <Modal.Body >
...
<div className="row">
 ...
 <h3 style={{ backgroundColor: '#EBF9ED', color: 'black', border: '2px solid black', borderRadius: '20px', padding: '5px', position: 'relative' }}>
  ...
  <div className="col">
    <div className="form-group">
          ...
          <select className="form-select" onChange={handlePartnerSelection}
          style={{ width: '200px' }}
          >
        {/* Populate options dynamically based on fetched partner data */}
  ...
  <div className="col">
    <Button className="remove-button no-background"  onClick={handleAddPartner} style={{ width: 'auto' }}>
    <span className="hover-text1" style={{ marginLeft: '5px' }}> Add New Partner</span>
 ...
 <Button onClick={() => handleAeSpocDialogButtonClick(true)} style={{  flex: '1', padding: '0px', height: '40px', borderRadius: '25px 25px 25px 25px'  ,border: 'none', background: 'none', position: 'relative', cursor: 'pointer'}}>
  ...
  <Modal show={showDialog} onHide={handleCloseDialog} dialogClassName="modal-lg">
        <Modal.Header closeButton>
          <Modal.Title>Add Customer SPOC</Modal.Title>
        </Modal.Header>
        <Modal.Body>
        <div className="row mb-3">
  <div className='col d-flex align-items-center'>
          ...
          <span className="hover-text1" style={{ marginLeft: '5px' }}> Add Customer SPOC</span>
 ...
 <div  className="row" >
    ...
    <FontAwesomeIcon icon={faUserPlus} style={{ fontSize: '24px', color: "green" }} />
    <span className="hover-text1" style={{ marginLeft: '5px' }}> Add New Customer SPOC</span>
    </Button>
...
<Modal show={showAeSpocDialog} onHide={handleCloseAeSpocDialog} dialogClassName="modal-lg">
  <Modal.Header >
    <Modal.Title>Customer SPOC Details</Modal.Title>
  </Modal.Header>
  <Modal.Body>
  <table className="table partner-table">
  <thead>
  ...
  <Modal.Footer>
    <Button variant="secondary" onClick={handleCloseAeSpocDialog}>
      Close
                ...
                <Modal show={showForm} onHide={handleClose2} dialogClassName="modal-lg">
  <Modal.Header >
    <Modal.Title>Add New Customer SPOC</Modal.Title>
  </Modal.Header>
  <Modal.Body >
...
{errors.Ae_Spoc_id && <div className="text-danger">{errors.Ae_Spoc_id}</div>}
  ...
  <Modal.Header closeButton>
    <Modal.Title>Add AE SPOC</Modal.Title>
  </Modal.Header>
  <Modal.Body>
  <div className="row mb-3">
  <div className='col d-flex align-items-center'>
  ...
  <Modal.Header >
    <Modal.Title>Customer AE SPOC Details</Modal.Title>
  </Modal.Header>
  <Modal.Body>
    <table className="table partner-table">
      <thead>
...
<div className="text-center">
  ...
  <Button variant="btn btn-danger" onClick={handleCancelButtonClick}>Cancel</Button>
      ...
      <Footer />
      <div>
      <Modal show={showPartnerModal} onHide={handleClose}>
      <Modal.Header closeButton>
  ...
  <div className="row mb-2">
    ...
    <div className="col-md-4 d-flex align-items-center">
      <Button className="remove-button no-background" onClick={handleClose} style={{ width: 'auto' }}>
        <FontAwesomeIcon icon={faUserPlus} className="icon" style={{ fontSize: '24px', color: 'green' }} />
      </Button>
    </div>
  </div>
  ...
  <Button variant="primary" onClick={handleProceedWithoutPartner} > 
    Proceed Without Partner
  </Button>
</Modal.Footer>
    </Modal>
</div>
