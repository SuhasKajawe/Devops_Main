...
const All_customer = () => {
  ...
  const fetchCustomerDetails = async () => {
    try {
      const response = await axios.get('http://192.168.10.42:3001/show_customers');
  ...
  const handleUpdateCustomer = async () => {
    try {
      await axios.put(`http://192.168.10.42:3001/update_customer/${selectedCustomer.cm_id}`, formData);
      setIsModalOpen(false);
 ...
 /* const handleDeleteCustomer = async (customerId) => {
  ...
  const handleDeleteCustomer = async (customerId) => {
    try {
      await axios.put(`http://192.168.10.42:3001/delete_customer/${customerId}`); 
      window.location.href = '/Show_Customer';
      fetchCustomerDetails();
    } catch (error) {
      console.error('Error deleting customer:', error);
...
<br/>
      ...
      <div className='col'>
            <label htmlFor="search" className="search-label"><h6> Customer Name:</h6> <span>      </span></label>
  </div>
        ...
        <table className="customer-table">
          <thead>
            <tr>
              ...
              <th style={{ width: '170px' }}>Partner ID</th>
          ...
          <tbody>
            {currentCustomers.filter(customer => customer.name.toLowerCase().includes(searchTerm.toLowerCase())).map(customer => (
              <tr key={customer.cm_id}>
                <td>{customer.cm_id}</td>
                ...
                <td className="action-buttons">
                  <button className="edit-button" onClick={() => handleEditCustomer(customer)}>
                    ...
                    <span className="hover-text1"  >Update Customer</span>
                  </button>
                  <span style={{ margin: '0 10px' }}></span>
                  <button className="delete-button" onClick={() => handleDeleteCustomer(customer.cm_id)}>
                    <FontAwesomeIcon icon={faUserTimes} style={{ fontSize: '24px', color: 'blue' }} />
                    <span className="hover-text1"   >Disable Customer</span>
                  </button>
                </td>
              </tr>
            ))}
...
<div className="pagination">
  ...
  <div className="modal" style={{ display: 'block', backgroundColor: 'rgba(0,0,0,0.5)', position: 'fixed', top: 0, left: 0, right: 0, bottom: 0 }}>
    <div className="modal-content" style={{ backgroundColor: '#fff', margin: '15% auto', padding: '20px', width: '50%', border: '1px solid #ccc' , border: '2px solid black', borderRadius: '20px', padding: '20px' }}>
      <h2 style={{ textAlign: 'center', marginBottom: '20px' , border: '2px solid black', borderRadius: '20px', padding: '20px' }}>{isEditing ? 'Edit Customer' : 'Update Customer'}</h2>
      <hr/>
